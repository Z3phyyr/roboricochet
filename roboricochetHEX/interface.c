#include "interface.h"

static const SDL_Color black = {0, 0, 0, 255};
static const SDL_Color white = {255, 255, 255, 255};
static const SDL_Color blue = {20, 20, 200, 255};


const int textX = 850;
const int textY = 50;
const int fontSize = 30;
const int wrappedSize = 300;

//const SDL_Rect effacement = {textX, textY + 2*fontSize, wrappedSize, fontSize};


static void SetColor (SDL_Renderer* renderer, SDL_Color color) {
	SDL_SetRenderDrawColor(renderer, color.r, color.g, color.b, color.a);
}

int EffaceLignes(SDL_Renderer* renderer, int lineNumber, int n) {
	SDL_Rect effacement = {textX, textY + lineNumber*fontSize, wrappedSize, n*fontSize};
	return SDL_RenderFillRect(renderer, &effacement);
}


int GetButtonTextTexture(SDL_Renderer* renderer, Button* button, const char* texte, int size, const char* font_path) {
	TTF_Font* hell = TTF_OpenFont(font_path, size);
	SDL_Surface* s = TTF_RenderText_Solid(hell, texte, blue);
	TTF_CloseFont(hell);

	if (s == NULL) return -1;

	button->shape.h = s->h;
	button->shape.w = s->w;

	button->texte = SDL_CreateTextureFromSurface(renderer, s);
	SDL_FreeSurface(s);

	if (button->texte == NULL) return -1;
	
	
	return 0;
}

int AfficherBouton(SDL_Renderer* renderer, Button* b) {
	
	SetColor(renderer, b->color);
	if (SDL_RenderFillRect(renderer, &b->shape) < 0) {
		return -1;
	}
	SetColor(renderer, b->outline_color);
	if (SDL_RenderDrawRect(renderer, &b->shape) < 0) {
		return -1;
	}
	if (SDL_RenderCopy(renderer, b->texte, NULL, &b->shape)) {
		return -1;
	}

	return 0;
}

void FreeButton(Button* b) {
	SDL_DestroyTexture(b->texte);
}


bool CheckBoutonPresse(Button* b, int x, int y) {
	return (inBounds(x, b->shape.x, b->shape.x + b->shape.w)
		&&  inBounds(y, b->shape.y, b->shape.y + b->shape.h));
}

int AffichageTexteInformatif(SDL_Renderer* renderer, int distance_totale) {
	const char* FixedSys = "C:/Program_Files/OCaml64/home/Admin/C/SDL/roboricochetSQUARE/fonts/FixedSys.ttf";
	char line1[140] = "Nombre de coups du chemin = ";
	char line2[115] = "Position 0 / ";
	
	
	
	char dist[100];
	sprintf(dist, "%d", distance_totale);
	strcat(line1, dist);

	strcat(line2, dist);

	//Efface ce qu'il y avait auparavant
	SetColor(renderer, white);
	if (EffaceLignes(renderer, 0, 3) != 0) return -1;


	if (PrintWrappedText(renderer, line1, FixedSys, 
			black, fontSize, textX, textY, wrappedSize) != 0 ||
		PrintWrappedText(renderer, line2, FixedSys, 
			black, fontSize, textX, textY + 2*fontSize, wrappedSize) != 0) {
				return -1;
	}
	return 0;
}

int AffichagePosition(SDL_Renderer* renderer, int distance, int distance_totale) {
	const char* FixedSys = "C:/Program_Files/OCaml64/home/Admin/C/SDL/roboricochetSQUARE/fonts/FixedSys.ttf";
	char line2[115] = "Position ";

	char dist_tot[100];
	char dist[100];

	sprintf(dist_tot, "%d", distance_totale);
	sprintf(dist, "%d", distance);

	strcat(line2, dist);
	strcat(line2, " / ");
	strcat(line2, dist_tot);

	//Efface ce qu'il y avait auparavant
	SetColor(renderer, white);
	if (EffaceLignes(renderer, 2, 1) != 0) return -1;

	if (PrintWrappedText(renderer, line2, FixedSys, black, fontSize, textX, textY + 2*fontSize, wrappedSize) != 0) {
		return -1;
	}
	return 0;
}